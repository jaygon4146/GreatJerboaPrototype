...
public class PixelMapLoader : MonoBehaviour
{
    ...
    public Transform LevelMasksTransform;
    ...
    void LoadMap()
    {
        ...
        for (int i = 0; i < mapLength; i++)
        {
            ...
            if (mapCells[i].getType() == (int)CellIDs.GrassBox)
            {   //If this is a box
            ...
            {                                       //If this is NOT a box
                ...
                if (VertGroupLengths[xPos] > 0)
                {                   //check the vert group count
            ...
            if (yPos == mapSize.y - 1)
            {                           //If we are at the end of a vertical line
                if (VertGroupLengths[xPos] > 0)
                {               //check that group count of the previous 
                    CreateVerticalGroupedBox(xPos);
                }
            }

            if (xPos == 0 && yPos > 0)
            {                           //If we are at the beginning of a new line
                if (HorzGroupLengths[yPos - 1] > 0)
                {               //check the group count of the previous line
        ...
        {
            MapCell cell = new MapCell(palette.getGrassBoxColor(), singleCandidates[i]);
            SpawnCell(cell);
    ...
    void CreateHorizontalGroupedBox(int y)
    {
        ...
        MapCell cell = new MapCell(palette.getGrassBoxColor(), v);
    ...
    private void SpawnMask(MapCell cell)
    {
        GameObject original = palette.lookUpMask();
        GameObject obj = Instantiate(
            original,                               //original
            cell.getPosition(),                     //position
            Quaternion.identity,                    //rotation
            LevelMasksTransform                     //parent				
        );
        string n = palette.lookUpName(cell.getType());
        obj.name = n + " @ " + cell.getPosition();
    }
